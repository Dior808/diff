...
db = Database("%s" % employee_db)
root = Tk()
root.title("Drivers Announcement System")
...
title = Label(entries_frame, text="Drivers Announcement System", font=("Calibri", 18, "bold"), bg="#535c68", fg="white")
...
lbldoj = Label(entries_frame, text="Last Name", font=("Calibri", 16), bg="#535c68", fg="white")
...
def getData(event):  # получить данные
...
def add_employee():
    if txtName.get() == "" or txtAge.get() == "" or txtDoj.get() == "" or txtEmail.get() == "" or comboGender.get() == "" or txtContact.get() == "" or txtAddress.get(
            1.0, END) == "":
        messagebox.showerror("Erorr in Input", "Please Fill All the Details")  # заполнение поли
        return
    db.insert(txtName.get(),txtAge.get(), txtDoj.get() , txtEmail.get() ,comboGender.get(), txtContact.get(), txtAddress.get(
            1.0, END))
    messagebox.showinfo("Success", "Record Inserted")  # вставить запись
    clearAll()
    dispalyAll()
...
def update_employee():
    if txtName.get() == "" or txtAge.get() == "" or txtDoj.get() == "" or txtEmail.get() == "" or comboGender.get() == "" or txtContact.get() == "" or txtAddress.get(
            1.0, END) == "":
        messagebox.showerror("Erorr in Input", "Please Select An ID Number")  # выбрать ID номер
        return
    db.update(row[0],txtName.get(), txtAge.get(), txtDoj.get(), txtEmail.get(), comboGender.get(), txtContact.get(),
              txtAddress.get(
                  1.0, END))
    messagebox.showinfo("Success", "Record Update")  # Обновление записи
    clearAll()
    dispalyAll()


def delete_employee():  # удалить сотрудника
    db.remove(row[0])
    clearAll()
    dispalyAll()


def clearAll(): # очистить все
    name.set("")
    age.set("")
    doj.set("")
    gender.set("")
...
btnAdd = Button(btn_frame, command=add_employee, text="Add Details", width=15, font=("Calibri", 16, "bold"), fg="white", # Добавить детали
                bg="#16a085", bd=0).grid(row=0, column=0)
btnEdit = Button(btn_frame, command=update_employee, text="Update Details", width=15, font=("Calibri", 16, "bold"),      # Обновить детали
                 fg="white", bg="#2980b9",
                 bd=0).grid(row=0, column=1, padx=10)
btnDelete = Button(btn_frame, command=delete_employee, text="Delete Details", width=15, font=("Calibri", 16, "bold"),    # Удалить детали
                   fg="white", bg="#c0392b",
                   bd=0).grid(row=0, column=2, padx=10)
btnClear = Button(btn_frame, command=clearAll, text="Clear Details", width=15, font=("Calibri", 16, "bold"), fg="white", # Очистить детали
                  bg="#f39c12",
...
style.configure("mystyle.Treeview", font=('Calibri', 18),
                rowheight=50)                                      # Изменить шрифт тела
style.configure("mystyle.Treeview.Heading", font=('Calibri', 18))  # Измените шрифт заголовков
...
tv['show'] = 'headings'
tv.bind("<ButtonRelease-1>", getData) #КнопкаВыпуск
tv.pack(fill=X)
